...
const Navbar = () => {
  ...
  const user = JSON.parse(localStorage.getItem('auth'))?.user.role === 0;
          ...
          <div className='flex w-1/2 items-center '>
              ...
              <div className='flex flex-row'>
                ...
                <div className='flex flex-col w-1/4 relative'>
                  <input
                    ...
                    className="w-96 p-2 font-medium bg-inherit focus:outline-none hover:placeholder-slate-400 placeholder-gray-500 text-white"
                  />
                </div>
            ...
            {isMobile && showDiv && (
              <div className='flex flex-col fixed top-0 right-0 h-full bg-white shadow-md p-4'>
                <p>{auth.name}</p>
                <NavLink to="/dashboard/user/books" className="text-black font-bold text-lg mb-4">Home</NavLink>
                <NavLink to="/dashboard/user/explore" className="text-black font-bold text-lg mb-4">Explore</NavLink>
            ...
            {!isMobile && (
              <div className='flex items-center space-x-4'>
                <NavLink to="/dashboard/user/books" className="text-white font-bold text-lg hover:bg-sky-200 hover:text-black transition duration-300">Home</NavLink>
                <NavLink to="/dashboard/user/explore" className="text-white font-bold text-lg hover:bg-sky-200 hover:text-black transition duration-300">Explore</NavLink>
              </div>
            )}
          </div>
          <div>
            ...
            {showDiv && (
              <div className='flex flex-col absolute top-full right-10 w-72 bg-white shadow-md mt-1 p-2 rounded-md hidden md:block'>
                <p className='ml-1'>{auth.name}</p>
